...
import numpy as np
import cv2
from openvino.inference_engine import IENetwork, IECore, InputInfoPtr
import sys
...
def test(_args):
    ...
    ie = IECore()
    net = ie.read_network(model=_args.xml, weights=_args.bin)
    ...
    for i, image_path in tqdm(enumerate(images_path), desc="testing..."):
        ...
        opvn_outs = exec_net.infer(inputs={input_blob: [img]})

        loc = opvn_outs["bbox"]
        conf = opvn_outs["cls"]
        landms = opvn_outs["ldm"]
        if cfg['gender']:
            gender = opvn_outs["gender"]
        ...
        name = os.path.join(_args.save, image_path.split('/')[-1].split('.')[0] + "_{}_opvn.jpg".format(cfg["name"]))
