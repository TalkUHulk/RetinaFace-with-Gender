...
def test(_args):
    ...
    onnx_model = onnx.load(_args.ckpt)
    onnx.checker.check_model(onnx_model)
    ort_session = onnxruntime.InferenceSession(_args.ckpt)
    ...
    for i, image_path in tqdm(enumerate(images_path), desc="testing..."):
        ...
        if cfg['gender']:
            loc, conf, landms, gender = ort_outs
        ...
        name = os.path.join(_args.save, image_path.split('/')[-1].split('.')[0] + "_{}_onnx.jpg".format(cfg["name"]))
