...
为了防止上面的情况, 我们将那些对于gtbox来说, 交并比最高的priorbox, 强制进行互相匹配, 即令 best_truth_idx[best_prior_idx[j]] = j, 详细见下面的for循环.
根据下标获取每个priorbox对应的gtbox的坐标, 然后对坐标进行相应编码, 并存储起来, 同时将gt类别也存储起来, 到此, 匹配完成.
...
def match_gender(threshold, truths, priors, variances, labels, landms, genders, loc_t, conf_t, landm_t, gender_t, idx):
    ...
    conf[best_truth_overlap < threshold] = 0  # label as background   overlap<0.35的全部>作为负样本

    loc = encode(matches, priors, variances)

    matches_landm = landms[best_truth_idx]
    gender = genders[best_truth_idx]
